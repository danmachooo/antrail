  ...
  <PanelShell step="4" title="Interactive Preview" subtitle="Simulated in-app overlay">
    <div class="space-y-4">
      ...
      <div ref="imsRef" class="relative overflow-hidden rounded-xl border border-slate-700 bg-slate-100">
        ...
        <div class="flex min-h-[320px]">
          <aside class="w-48 border-r border-slate-200 bg-white p-3">
            ...
            <button
              ...
              :class="item.id === 'sidebar-inventory' ? 'bg-cyan-50 text-cyan-700' : 'text-slate-600 hover:bg-slate-100'"
            >
          ...
          <main class="flex-1 p-5">
            <div class="mb-4 flex items-center justify-between">
              ...
              <button id="ims-add-btn" class="rounded-lg bg-cyan-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-cyan-700">
                Add Item
              </button>
            </div>

            <div id="ims-table" class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <table class="w-full border-collapse text-xs">
                ...
                <tbody>
                  <tr v-for="item in IMS_ITEMS" :key="item.sku" class="border-b border-slate-100 last:border-b-0">
                    ...
                    <td class="px-3 py-2">
                      <span
                        ...
                        :class="item.status === 'In Stock' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'"
                      >
                        {{ item.status }}
                      </span>
                    </td>
        ...
        <div v-if="tourActive && highlightStyle" class="pointer-events-none absolute z-10 rounded-lg shadow-[0_0_0_3px_rgba(16,185,129,1),0_0_0_7px_rgba(16,185,129,0.2)] transition-all duration-300" :style="highlightStyle" />
          ...
          >
            ...
            <div class="mt-4 flex items-center gap-2">
              ...
              <AppButton size="sm" @click="tourNext">
                <component :is="tourIndex >= store.steps.length - 1 ? Check : ArrowRight" class="h-3.5 w-3.5" />
