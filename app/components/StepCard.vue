	...
	>
		<template v-if="!isEditing">
			<div class="flex items-start gap-3">
				<div
					class="mt-0.5 inline-flex h-8 w-8 flex-none items-center justify-center rounded-md border border-cyan-400/30 bg-cyan-500/10 text-xs font-bold text-cyan-200"
				>
					{{ step.stepNumber }}
				</div>
				...
				<div class="min-w-0 flex-1">
					...
					<div class="mt-3 flex flex-wrap gap-2">
						<span
							class="inline-flex items-center gap-1 rounded-md border border-emerald-400/30 bg-emerald-500/10 px-2 py-1 text-[11px] text-emerald-200"
						>
						...
						<span
							class="rounded-md border border-amber-400/30 bg-amber-500/10 px-2 py-1 font-mono text-[11px] text-amber-200"
						>
							{{ step.confirmedSelector ?? step.suggestedSelector ?? "no selector" }}
						</span>
						<span class="rounded-md border px-2 py-1 text-[11px]" :class="confidenceClass">
